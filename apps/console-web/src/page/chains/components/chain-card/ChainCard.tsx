...
type ChainCardProps = {
  chain: TChain;
  onEdit: (chain: TChain) => void;
};

const ChainCard: React.FC<ChainCardProps> = ({ chain, onEdit }) => {
  ...
  return (
    <StyledCard>
      ...
      <StyledCardContent>
        ...
        <StyledAddress variant="h6">{`Emirate: ${chain.emirate}`}</StyledAddress>
        <StyledCode variant="body2">{`City: ${chain.city}`}</StyledCode>
      </StyledCardContent>

      {/* Lower portion (30%) */}
      <StyledLowerBox>
        <StyledCTABox>
          <StatusPill status={"close"} />
        </StyledCTABox>
